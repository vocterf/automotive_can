...
// DOŁĄCZENIE NASZEGO MODUŁU SIECIOWEGO
...
double process_camera_image(const unsigned char *image) {
  ...
  const unsigned char REF[3] = {95, 187, 203}; // BGR
  int sumx = 0;
  int pixel_count = 0;
...
double filter_angle(double new_value) {
  ...
  if (first_call || new_value == UNKNOWN) {
    first_call = false;
    for (int i = 0; i < FILTER_SIZE; ++i) old_value[i] = 0.0;
  } else {
    for (int i = 0; i < FILTER_SIZE - 1; ++i) old_value[i] = old_value[i + 1];
  }
  if (new_value == UNKNOWN) return UNKNOWN;
  old_value[FILTER_SIZE - 1] = new_value;
  double sum = 0.0;
  for (int i = 0; i < FILTER_SIZE; ++i) sum += old_value[i];
...
void compute_gps_speed() {
  ...
  gps_speed = wb_gps_get_speed(gps) * 3.6;
...
int main(int argc, char **argv) {
  ...
  // INICJALIZACJA NASZEJ NIEBLOKUJĄCEJ SIECI UDP (Localhost, port 8080)
  init_network("127.0.0.1", 8080);
  ...
  while (wbu_driver_step() != -1) {
    ...
    if (i % (int)(TIME_STEP / wb_robot_get_basic_time_step()) == 0) {
      ...
      if (autodrive && has_camera) {
        ...
        if (enable_collision_avoidance) obstacle_angle = process_sick_data(sick_data, &obstacle_dist);
        else { obstacle_angle = UNKNOWN; obstacle_dist = 0; }
    ...
    // --- STRZELANIE TELEMETRIĄ I ODBIÓR ASYNCHRONICZNY Z RUSTA ---
    double current_speed = wbu_driver_get_current_speed();
    double current_rpm = wbu_driver_get_rpm();
    
    // Wysyłamy nieblokująco
    send_telemetry(current_speed, current_rpm);

    // Sprawdzamy, czy w locie coś przyszło z Rusta. Jeśli nie - if się pomija i jedziemy dalej.
    ControlPacket rx_packet;
    if (receive_control(&rx_packet)) {
        set_speed((double)rx_packet.target_speed);
        wbu_driver_set_brake_intensity((double)rx_packet.brake_intensity / 100.0);
        printf("[ECU OVERRIDE] Target Speed: %f km/h, Brake: %f%%\n", rx_packet.target_speed, rx_packet.brake_intensity);
    }

    ++i;
  }

  // ZWOLNIENIE PAROBKÓW SIECIOWYCH NA KONCU
  close_network();
  wbu_driver_cleanup();
  return 0;
}